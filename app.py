...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V2.6</title>
<style>
...
<h1>📈 NIFTY Professional Signals V2.6</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
<script>
"use strict";const $=id=>document.getElementById(id);const fmt=x=>(x==null||!Number.isFinite(Number(x)))?"—":Number(x).toLocaleString("en-IN",{minimumFractionDigits:2,maximumFractionDigits:2});let busy=false,notificationsEnabled=localStorage.getItem("niftyV26Notify")==="1";
...
function readTrade(){try{return JSON.parse(localStorage.getItem("niftyV26Trade")||"null")}catch(e){return null}}
function saveTrade(t){localStorage.setItem("niftyV26Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV26Trade");updateTradeUI(null)}
...
function renderLockedTrade(t,q=null){
 ...
 $("liquidity").textContent=q
   ?`LOCKED CONTRACT • Volume ${q.volume||0} • OI ${q.oi||0} • BidQty ${q.bid_qty||0} • AskQty ${q.ask_qty||0}${q.spread_pct!=null?` • Spread ${fmt(q.spread_pct)}%`:""}`
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V2.6";$("error").textContent=d.warning||"";
...
def exact_option_quote(strike,opt_type,expiry_date):
    """Return a live quote for the exact contract already locked in the trade monitor."""
    ...
    for row in rec.get("data",[]):
        if float(row.get("strikePrice",0) or 0)!=strike:continue
        if expiry_date and row.get("expiryDate")!=expiry_date:continue
        ...
        return {
            ...
            "expiry":row.get("expiryDate") or expiry_date,
    ...
    raise RuntimeError("Locked option contract not found in current NSE option chain.")
