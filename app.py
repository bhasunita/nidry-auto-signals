...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V2.8</title>
<style>
...
<h1>📈 NIFTY Professional Signals V2.8</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
if(!localStorage.getItem("niftyV28Notify")){
 const oldNotify=localStorage.getItem("niftyV27Notify")||localStorage.getItem("niftyV26Notify");
 if(oldNotify==="1")localStorage.setItem("niftyV28Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV28Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV28Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV27Trade")||localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV28Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV28Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV28Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");updateTradeUI(null)}
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"ENTRY REFERENCE LOCKED • SL/TARGET MONITORING • NEW SIGNALS HIDDEN";
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V2.8";$("error").textContent=d.warning||"";
