...
import requests, time, math, os, secrets
...
# V2.7 PRIVATE ACCESS
# Set these in Render -> Environment. Do NOT hard-code your password in GitHub.
APP_USER = os.environ.get("APP_USER", "owner")
APP_PASSWORD = os.environ.get("APP_PASSWORD", "")

@app.before_request
def require_private_access():
    # Keep a tiny health endpoint public so Render can verify the service.
    if request.path == "/health":
        return None

    # Fail closed: if no password is configured, the dashboard is NOT public.
    if not APP_PASSWORD:
        return Response(
            "Private access is not configured. Set APP_PASSWORD in Render Environment.",
            503,
            {"Content-Type":"text/plain; charset=utf-8"}
        )

    auth = request.authorization
    user_ok = bool(auth) and secrets.compare_digest(auth.username or "", APP_USER)
    pass_ok = bool(auth) and secrets.compare_digest(auth.password or "", APP_PASSWORD)
    if not (user_ok and pass_ok):
        return Response(
            "Private NIFTY dashboard. Login required.",
            401,
            {
                "WWW-Authenticate": 'Basic realm="NIFTY Private Dashboard"',
                "Content-Type":"text/plain; charset=utf-8"
            }
        )
    return None
...
def health():
    return jsonify({"ok": True, "version": "2.7"})
...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V2.7</title>
<style>
...
<h1>📈 NIFTY Professional Signals V2.7</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
if(!localStorage.getItem("niftyV27Notify")&&localStorage.getItem("niftyV26Notify")==="1")localStorage.setItem("niftyV27Notify","1");
let notificationsEnabled=localStorage.getItem("niftyV27Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV27Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV27Trade",old);raw=old}
 }
 return JSON.parse(raw||"null")
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV27Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");updateTradeUI(null)}
...
function updateTradeUI(t){
 ...
 const cp=Number(t.currentLtp||0)>0?` • Current ₹${fmt(t.currentLtp)}`:"";
 $("tradeDetail").textContent=`${t.contract} • Locked Entry ₹${fmt(t.entry)}${cp} • SL ₹${fmt(t.sl)} • T1 ₹${fmt(t.t1)} • T2 ₹${fmt(t.t2)}`
}
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"LOCKED TRADE MONITORING • NEW SIGNALS HIDDEN";
...
function applyTradePrice(t,p){
 ...
 if(t.state==="ORDER READY / WAITING FOR ENTRY" && p>=t.entry){
   ...
   notify("🔔 ORDER PRICE HIT",`${t.contract} • LTP ₹${fmt(p)} reached locked entry ₹${fmt(t.entry)} • SL ₹${fmt(t.sl)}`,"entry");
 }
...
function monitorTrade(d){
 ...
 notify("✅ SIGNAL CONFIRMED",`${t.contract} • LOCKED Entry ₹${fmt(t.entry)} • SL ₹${fmt(t.sl)} • T1 ₹${fmt(t.t1)} • T2 ₹${fmt(t.t2)}`,"setup");

 // If LTP is already at/above locked ask on the confirmation refresh, mark entry hit.
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V2.7";$("error").textContent=d.warning||"";

 const t=readTrade();
 if(t){
   // Do not show fresh candidate entry/SL/targets over an existing trade.
   renderLockedTrade(t,null);
   monitorTrade(d);
   return;
 }
