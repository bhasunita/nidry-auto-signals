...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V2.2</title>
<style>
...
<h1>📈 NIFTY Professional Signals V2.2</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
function readTrade(){try{return JSON.parse(localStorage.getItem("niftyV22Trade")||"null")}catch(e){return null}}function saveTrade(t){localStorage.setItem("niftyV22Trade",JSON.stringify(t))}function clearTrade(){localStorage.removeItem("niftyV22Trade");updateTradeUI(null)}
...
function monitorTrade(d){if(!d.option)return;let t=readTrade();const active=Boolean(d.execution_ready&&d.option.tradable);if(!t&&active){t={contract:d.option.contract,entry:d.option.entry,sl:d.option.sl,t1:d.option.target1,t2:d.option.target2,state:"WAITING FOR ENTRY"};saveTrade(t);updateTradeUI(t);notify("NEW TRADE SETUP",`${t.contract} • Entry ₹${fmt(t.entry)}`)}if(!t||d.option.contract!==t.contract)return;const p=Number(d.option.ltp);if(!Number.isFinite(p)||p<=0)return;
...
$("dot").className="dot on";$("status").textContent="Connected • V2.2";$("error").textContent=d.warning||"";monitorTrade(d);updateTradeUI(readTrade())}
...
def choose_option(oc,spot,bull):
    ...
    for row in rec["data"]:
        ...
        # Search a wider nearby band so a bad ATM quote can be skipped.
        if abs(st-atm)>250:continue
        l=float(side.get("lastPrice",0) or 0)
        b=float(side.get("bidprice",side.get("bidPrice",0)) or 0)
        a=float(side.get("askPrice",side.get("askprice",0)) or 0)
        v=int(side.get("totalTradedVolume",0) or 0)
        oi=int(side.get("openInterest",0) or 0)

        # V2.2 safety gate: never build a trade from missing/zero/crossed quotes.
        if l<=0 or b<=0 or a<=0 or a<b:continue
        mid=(a+b)/2
        sp=(a-b)/mid*100 if mid>0 else 999

        # Reject clearly unusable quotes instead of treating LTP as an entry.
        if sp>12 or v<100 or oi<=0:continue

        # Prefer ATM, then tight spread, then volume/open interest.
        rank=(abs(st-atm)/50)*7 + sp*3 - min(math.log10(v+1),6)*4 - min(math.log10(oi+1),7)*3
        c.append((rank,st,l,b,a,v,oi,sp))

    if not c:
        raise RuntimeError("No nearby option with valid bid/ask and acceptable liquidity.")

    _,st,l,b,a,v,oi,sp=sorted(c)[0]
    entry=a
    spread=a-b
    risk=min(max(entry*.18,spread*2),entry*.25)
    liq="GOOD" if sp<=5 and v>=1000 else "FAIR"
    return {
...
def build_signal():
    ...
    buy_above=round(tv["high5"]+.1*tv["atr5"],2)
    sell_below=round(tv["low5"]-.1*tv["atr5"],2)
    ...
    trigger_hit=(tv["spot"]>=buy_above) if bull else (tv["spot"]<=sell_below)
    ...
    # V2.2: a strong directional score is only actionable after both
    # the NIFTY price trigger and a valid option bid/ask quote are confirmed.
    if strong:
        execution_ready=bool(trigger_hit and opt and opt.get("tradable"))
        if not execution_ready:
            signal=("BUY WATCH" if bull else "SELL WATCH")
            waiting=[]
            if not trigger_hit:waiting.append("NIFTY trigger")
    ...
    return {
        ...
        "reason":f"{conf}/100 • 5m {tv_rating(tv['rec5'])} • 15m {tv_rating(tv['rec15'])} • ADX {tv['adx5']:.1f}",
        ...
        "trigger_hit":trigger_hit,"market_open":market_open_now(),
        "data_source":"TradingView + NSE","warning":warn,
        "updated":datetime.now(IST).strftime("%d-%b %I:%M:%S %p")
    }
