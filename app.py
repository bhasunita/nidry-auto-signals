...
def health():
    return jsonify({"ok": True, "version": "3.4"})
...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V3.4</title>
<style>
...
<h1>📈 NIFTY Professional Signals V3.4</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
<div class="small">V3.4 NEW-ENTRY QUALITY ENGINE</div>
...
const LAST_SIGNAL_KEY="niftyV34LastSignal";
...
function showConnectionIssue(msg,nextSeconds=null){
 ...
 $("status").textContent="Reconnecting • V3.4";
}
...
if(!localStorage.getItem("niftyV34Notify")){
 ...
 if(oldNotify==="1")localStorage.setItem("niftyV34Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV34Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV34Trade");
 if(!raw){
   ...
   if(old){localStorage.setItem("niftyV34Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV34Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV34Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV34Trade");localStorage.removeItem("niftyV33Trade");localStorage.removeItem("niftyV32Trade");localStorage.removeItem("niftyV31Trade");localStorage.removeItem("niftyV30Trade");localStorage.removeItem("niftyV29Trade");localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");localStorage.removeItem("niftyV34Reversal");hideReversalWarning();updateTradeUI(null)}
...
function evaluateReversalWarning(t,d){
 ...
 if(!strongConflict){
   hideReversalWarning();
   localStorage.removeItem("niftyV34Reversal");
   return false;
 }
 ...
 const prior=localStorage.getItem("niftyV34Reversal");
 if(prior!==key){
   localStorage.setItem("niftyV34Reversal",key);
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"LOCKED TRADE • LIVE P/L + SL/TARGET MONITORING • V3.4 QUALITY + REVERSAL CHECKS ACTIVE";
...
function render(d){
 ...
 $("qualityGrade").textContent=qGrade==="A"?"GRADE A • CONFIRMED SETUP":qGrade==="B"?"GRADE B • PREPARE ONLY":"NO NEW ENTRY";
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V3.4";$("error").textContent=d.warning||"";
 saveLastSignal(d);
 clearConnectionIssue();

 const t=readTrade();
 if(t){
   // V3.4: locked trade remains separate; scanner drives quality and reversal warnings.
...
def fetch_tv():
    # V3.4 adds trend, momentum and market-regime confirmation.
    cols=[
...
def score(tv,bull=True):
    """
    V3.4 deliberately favors fewer, higher-quality setups.
...
def quality_gate(tv,bull,score_value):
    """V3.4 separates setup quality from the existing locked-trade monitor.
...
def build_signal():
    ...
    # V3.4: score alone can never create an entry.
