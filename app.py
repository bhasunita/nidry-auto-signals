# NIFTY Professional Signals V4.0
...
# STATE_FILE=/tmp/nifty_v40_state.json
#
# IMPORTANT:
...
log = logging.getLogger("nifty-v40")
...
STATE_FILE = os.environ.get("STATE_FILE", "/tmp/nifty_v40_state.json")
    ...
    try:
        if os.path.exists(STATE_FILE):
            ...
            if isinstance(data, dict):
                for k in ("trade", "journal", "last_alert_keys", "last_event"):
                    if k in data:
                        _state[k] = data[k]
...
def evaluate_reversal(t, d):
    ...
    if strong_conflict and not t.get("reversal_alerted"):
        ...
        send_whatsapp(
            "NIFTY V4.0 - REVERSAL / EXIT REVIEW",
            (
...
def apply_trade_price(t, p, d):
    ...
    if t.get("target1_hit") and not is_trade_closed(t):
        ...
        if p >= stage2 and float(t.get("trailing_sl") or 0) < t1:
            ...
            send_whatsapp(
                "NIFTY V4.0 - TRAILING STOP UPDATED",
    ...
    if t.get("target1_hit") and not t.get("add_on_eligible") and not is_trade_closed(t):
        ...
        if same and int(d["confidence"]) >= 92 and d["market_regime"] == "TRENDING":
            ...
            send_whatsapp(
                "NIFTY V4.0 - ADD-ON ELIGIBLE / REVIEW ONLY",
                (
...
def background_loop():
    log.info("V4.0 background monitor started.")
    time.sleep(4)
...
<title>NIFTY Professional Signals V4.0</title>
<style>
:root{
...
<div class="title">NIFTY Professional Signals V4.0</div>
...
<div class="card">
  <div class="small">V4.0 PROFESSIONAL ENTRY QUALITY ENGINE</div>
...
<div class="card">
  <div class="small">V4.0 SERVER TRADE MONITOR</div>
...
<div class="card">
  <div class="small">V4.0 PROFESSIONAL TRADE HEALTH</div>
...
<div class="card">
  <div class="small">V4.0 END-OF-DAY POSITION DECISION</div>
...
<div class="card">
  <div class="small">V4.0 SESSION JOURNAL</div>
  <div class="grid2" style="margin-top:12px">
...
function browserAlertForEvent(s){
  ...
  if("Notification" in window && Notification.permission==="granted"){
    try{
      new Notification("NIFTY V4.0 - "+e.kind,{body:(e.contract||"")+" "+(e.detail||"")});
    }catch(err){}
  }
}
...
function renderSignal(d){
  ...
  $("status").textContent="Connected - V4.0";
