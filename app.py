...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V3.1</title>
<style>
...
<h1>📈 NIFTY Professional Signals V3.1</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
const LAST_SIGNAL_KEY="niftyV31LastSignal";
...
function showConnectionIssue(msg,nextSeconds=null){
 ...
 $("status").textContent="Reconnecting • V3.1";
}
...
if(!localStorage.getItem("niftyV31Notify")){
 const oldNotify=localStorage.getItem("niftyV30Notify")||localStorage.getItem("niftyV29Notify")||localStorage.getItem("niftyV28Notify")||localStorage.getItem("niftyV27Notify")||localStorage.getItem("niftyV26Notify");
 if(oldNotify==="1")localStorage.setItem("niftyV31Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV31Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV31Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV30Trade")||localStorage.getItem("niftyV29Trade")||localStorage.getItem("niftyV28Trade")||localStorage.getItem("niftyV27Trade")||localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV31Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV31Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV31Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV31Trade");localStorage.removeItem("niftyV30Trade");localStorage.removeItem("niftyV29Trade");localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");updateTradeUI(null)}
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"LOCKED TRADE • LIVE P/L + SL/TARGET MONITORING • CURRENT MARKET VIEW SHOWN SEPARATELY";
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V3.1";$("error").textContent=d.warning||"";
 saveLastSignal(d);
