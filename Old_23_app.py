...
def health():
    return jsonify({"ok": True, "version": "3.3"})
...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V3.3</title>
<style>
...
<h1>📈 NIFTY Professional Signals V3.3</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
<div class="small">V3.3 HIGH-SELECTIVITY TRADE FILTER</div>
...
const LAST_SIGNAL_KEY="niftyV33LastSignal";
...
function showConnectionIssue(msg,nextSeconds=null){
 ...
 $("status").textContent="Reconnecting • V3.3";
}
...
if(!localStorage.getItem("niftyV33Notify")){
 const oldNotify=localStorage.getItem("niftyV32Notify")||localStorage.getItem("niftyV31Notify")||localStorage.getItem("niftyV30Notify")||localStorage.getItem("niftyV29Notify")||localStorage.getItem("niftyV28Notify")||localStorage.getItem("niftyV27Notify")||localStorage.getItem("niftyV26Notify");
 if(oldNotify==="1")localStorage.setItem("niftyV33Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV33Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV33Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV32Trade")||localStorage.getItem("niftyV31Trade")||localStorage.getItem("niftyV30Trade")||localStorage.getItem("niftyV29Trade")||localStorage.getItem("niftyV28Trade")||localStorage.getItem("niftyV27Trade")||localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV33Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV33Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV33Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV33Trade");localStorage.removeItem("niftyV32Trade");localStorage.removeItem("niftyV31Trade");localStorage.removeItem("niftyV30Trade");localStorage.removeItem("niftyV29Trade");localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");localStorage.removeItem("niftyV33Reversal");hideReversalWarning();updateTradeUI(null)}
...
function evaluateReversalWarning(t,d){
 ...
 if(!strongConflict){
   hideReversalWarning();
   localStorage.removeItem("niftyV33Reversal");
   return false;
 }
 ...
 const prior=localStorage.getItem("niftyV33Reversal");
 if(prior!==key){
   localStorage.setItem("niftyV33Reversal",key);
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"LOCKED TRADE • LIVE P/L + SL/TARGET MONITORING • V3.3 QUALITY + REVERSAL CHECKS ACTIVE";
...
function render(d){
 ...
 $("qualityGrade").textContent=qGrade==="A"?"GRADE A • TRADE QUALITY PASS":qGrade==="B"?"GRADE B • WATCH ONLY":"BLOCKED • NO TRADE";
 $("qualityGrade").style.color=qGrade==="A"?"#00d66b":qGrade==="B"?"#ffb020":"#ff7690";
 $("qualitySummary").textContent=`Hard filters passed ${d.quality_passed||0}/${d.quality_total||7}. V3.3 intentionally rejects weak/low-trend setups.`;
 $("qualityChecks").innerHTML=(d.quality_checks||[]).map(x=>`<span class="pill ${x.ok?"ok":"bad"}">${x.ok?"✓":"✕"} ${x.label}</span>`).join("");
 $("dot").className="dot on";$("status").textContent="Connected • V3.3";$("error").textContent=d.warning||"";
 saveLastSignal(d);
 clearConnectionIssue();

 const t=readTrade();
 if(t){
   // V3.3: locked trade remains separate; scanner drives quality and reversal warnings.
...
async function refresh(){
 ...
 try{
   ...
   if(cached&&cached.data){
     ...
     $("updated").textContent=`LAST KNOWN DATA • saved about ${ageMin} min ago`;
   }
...
def fetch_tv():
    # V3.3 adds trend, momentum and market-regime confirmation.
    cols=[
        "close|5","Recommend.All|5","RSI|5","EMA10|5","EMA20|5","EMA50|5",
        "MACD.macd|5","MACD.signal|5","ADX|5","ATR|5","high|5","low|5","VWAP|5",
        "Recommend.All|15","RSI|15","EMA10|15","EMA20|15","EMA50|15",
...
def score(tv,bull=True):
    """
    V3.3 deliberately favors fewer, higher-quality setups.
...
def quality_gate(tv,bull,score_value):
    ...
    regime_ok=tv["adx5"]>=20 and tv["adx15"]>=18
    rsi_ok=((52<=tv["rsi5"]<=68) and (50<=tv["rsi15"]<=68)) if bull else ((32<=tv["rsi5"]<=48) and (32<=tv["rsi15"]<=50))

    hard={
        "5m + 15m direction agree":same_direction,
        "EMA trend aligned":ema_trend,
        "Price aligned with EMA50":long_trend,
        "Price on correct side of VWAP":vwap_ok,
        "MACD confirms both timeframes":macd_ok,
        "Trend strength ADX 5m/15m":regime_ok,
        "RSI not weak/overextended":rsi_ok,
    }
    passed=sum(1 for v in hard.values() if v)
    strong_pass=all(hard.values()) and score_value>=80
    watch_pass=same_direction and ema_trend and long_trend and vwap_ok and regime_ok and score_value>=65
    return strong_pass,watch_pass,hard,passed
...
def build_signal():
    ...
    strong_gate,watch_gate,quality_checks,quality_passed=quality_gate(tv,bull,conf)

    # V3.3: score alone can no longer create a trade.
    # Strong signals require multi-timeframe trend + VWAP + EMA50 + MACD + ADX + RSI alignment.
    ...
    return {
        ...
        "reason":f"{conf}/100 • 5m {tv_rating(tv['rec5'])} • 15m {tv_rating(tv['rec15'])} • ADX 5m {tv['adx5']:.1f} / 15m {tv['adx15']:.1f} • Quality {quality_passed}/{len(quality_checks)} • Trigger {confirm_count}/2 • {trigger_note}",
        ...
        "quality_grade":("A" if strong_gate else "B" if watch_gate else "BLOCKED"),
