...
def fetch_oc():
    s=requests.Session();s.get(NSE_HOME,headers=NSE_HEADERS,timeout=10);s.get(NSE_OC_PAGE,headers=NSE_HEADERS,timeout=10)
    r=s.get(NSE_OC_API,headers=NSE_HEADERS,timeout=12);r.raise_for_status();j=r.json()
...
def choose_option(oc,spot,bull):
    ...
    for row in rec["data"]:
        if row.get("expiryDate")!=ex or not row.get(typ):continue
