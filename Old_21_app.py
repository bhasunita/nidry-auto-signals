...
def health():
    return jsonify({"ok": True, "version": "3.2"})
...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V3.2</title>
<style>
...
<h1>📈 NIFTY Professional Signals V3.2</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
const LAST_SIGNAL_KEY="niftyV32LastSignal";
...
function showConnectionIssue(msg,nextSeconds=null){
 ...
 $("status").textContent="Reconnecting • V3.2";
}
...
if(!localStorage.getItem("niftyV32Notify")){
 const oldNotify=localStorage.getItem("niftyV31Notify")||localStorage.getItem("niftyV30Notify")||localStorage.getItem("niftyV29Notify")||localStorage.getItem("niftyV28Notify")||localStorage.getItem("niftyV27Notify")||localStorage.getItem("niftyV26Notify");
 if(oldNotify==="1")localStorage.setItem("niftyV32Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV32Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV32Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV31Trade")||localStorage.getItem("niftyV30Trade")||localStorage.getItem("niftyV29Trade")||localStorage.getItem("niftyV28Trade")||localStorage.getItem("niftyV27Trade")||localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV32Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV32Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV32Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV32Trade");localStorage.removeItem("niftyV31Trade");localStorage.removeItem("niftyV30Trade");localStorage.removeItem("niftyV29Trade");localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");localStorage.removeItem("niftyV32Reversal");hideReversalWarning();updateTradeUI(null)}
...
function evaluateReversalWarning(t,d){
 ...
 if(!strongConflict){
   hideReversalWarning();
   localStorage.removeItem("niftyV32Reversal");
   return false;
 }
 ...
 const prior=localStorage.getItem("niftyV32Reversal");
 if(prior!==key){
   localStorage.setItem("niftyV32Reversal",key);
...
function renderLockedTrade(t,q=null){
 ...
 $("bias").textContent=isClosedTrade(t)?"LOCKED TRADE COMPLETE • RESET TO SCAN AGAIN":"LOCKED TRADE • LIVE P/L + SL/TARGET MONITORING • REVERSAL SAFETY CHECK ACTIVE";
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V3.2";$("error").textContent=d.warning||"";
 saveLastSignal(d);
 clearConnectionIssue();

 const t=readTrade();
 if(t){
   // V3.2: locked trade remains separate; current scanner also drives reversal warnings.
