...
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V3.0</title>
<style>
...
<h1>📈 NIFTY Professional Signals V3.0</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
if(!localStorage.getItem("niftyV30Notify")){
 const oldNotify=localStorage.getItem("niftyV29Notify")||localStorage.getItem("niftyV28Notify")||localStorage.getItem("niftyV27Notify")||localStorage.getItem("niftyV26Notify");
 if(oldNotify==="1")localStorage.setItem("niftyV30Notify","1");
}
let notificationsEnabled=localStorage.getItem("niftyV30Notify")==="1";
...
function readTrade(){
try{
 let raw=localStorage.getItem("niftyV30Trade");
 if(!raw){
   const old=localStorage.getItem("niftyV29Trade")||localStorage.getItem("niftyV28Trade")||localStorage.getItem("niftyV27Trade")||localStorage.getItem("niftyV26Trade");
   if(old){localStorage.setItem("niftyV30Trade",old);raw=old}
 }
 ...
 if(t){
   ...
   if(t.state==="ORDER READY / WAITING FOR ENTRY" || t.state==="ENTRY / ORDER PRICE HIT"){
     ...
     t.entryLocked=true;
     localStorage.setItem("niftyV30Trade",JSON.stringify(t));
   }
 }
 return t
}catch(e){return null}
}
function saveTrade(t){localStorage.setItem("niftyV30Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV30Trade");localStorage.removeItem("niftyV29Trade");localStorage.removeItem("niftyV28Trade");localStorage.removeItem("niftyV27Trade");localStorage.removeItem("niftyV26Trade");updateTradeUI(null)}
...
function render(d){
 ...
 $("dot").className="dot on";$("status").textContent="Connected • V3.0";$("error").textContent=d.warning||"";
