...
_trigger_state={"direction":None,"level":None,"count":0,"confirmed":False}

PAGE=r"""<!doctype html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<meta name="theme-color" content="#07111f"><title>NIFTY Professional V2.4</title>
<style>
...
<h1>📈 NIFTY Professional Signals V2.4</h1><div class="sub">5m + 15m confirmation • confidence scoring • option liquidity • trade alerts</div>
...
function readTrade(){try{return JSON.parse(localStorage.getItem("niftyV24Trade")||"null")}catch(e){return null}}
function saveTrade(t){localStorage.setItem("niftyV24Trade",JSON.stringify(t))}
function clearTrade(){localStorage.removeItem("niftyV24Trade");updateTradeUI(null)}
...
function updateTradeUI(t){
 ...
 $("tradeDetail").textContent=`${t.contract} • Locked Entry ₹${fmt(t.entry)} • SL ₹${fmt(t.sl)} • T1 ₹${fmt(t.t1)} • T2 ₹${fmt(t.t2)}`
}

function monitorTrade(d){
 if(!d.option)return;
 let t=readTrade();
 const active=Boolean(d.execution_ready&&d.trigger_confirmed&&d.option.tradable);

 // First confirmed setup: lock actual option ask + risk levels.
 if(!t&&active){
   t={
     ...
     state:"ORDER READY / WAITING FOR ENTRY",
     locked:true,
     lockedAt:d.updated
   };
   saveTrade(t);updateTradeUI(t);
   notify("✅ SIGNAL CONFIRMED",`${t.contract} • Order/entry ₹${fmt(t.entry)} • SL ₹${fmt(t.sl)} • T1 ₹${fmt(t.t1)} • T2 ₹${fmt(t.t2)}`,"setup");
 }

 if(!t||d.option.contract!==t.contract)return;
 const p=Number(d.option.ltp);
 if(!Number.isFinite(p)||p<=0)return;

 // Entry/order price hit
 if(t.state==="ORDER READY / WAITING FOR ENTRY" && p>=t.entry){
   t.state="ENTRY / ORDER PRICE HIT";
   saveTrade(t);updateTradeUI(t);
   notify("🔔 ORDER PRICE HIT",`${t.contract} • LTP ₹${fmt(p)} reached entry ₹${fmt(t.entry)} • SL ₹${fmt(t.sl)}`,"entry");
 }

 // Stop loss has priority after entry
 if((t.state==="ENTRY / ORDER PRICE HIT"||t.state==="TARGET 1 HIT") && p<=t.sl){
   t.state="STOP LOSS HIT / CLOSED";
   saveTrade(t);updateTradeUI(t);
   notify("🛑 STOP LOSS HIT",`${t.contract} • LTP ₹${fmt(p)} • SL ₹${fmt(t.sl)}`,"stop");
 }
 else if((t.state==="ENTRY / ORDER PRICE HIT"||t.state==="TARGET 1 HIT") && p>=t.t2){
   t.state="TARGET 2 HIT / CLOSED";
   saveTrade(t);updateTradeUI(t);
   notify("🏆 TARGET 2 HIT",`${t.contract} • LTP ₹${fmt(p)} • Final target ₹${fmt(t.t2)}`,"target2");
 }
 else if(t.state==="ENTRY / ORDER PRICE HIT" && p>=t.t1){
   t.state="TARGET 1 HIT";
   saveTrade(t);updateTradeUI(t);
   notify("✅ TARGET 1 HIT",`${t.contract} • LTP ₹${fmt(p)} • T1 ₹${fmt(t.t1)} • Next ₹${fmt(t.t2)}`,"target1");
 }
}
...
if(d.option){$("contract").textContent=d.option.contract;$("contract").className="contract "+(d.option.type==="CE"?"buy":"sell");$("expiry").textContent=`Expiry ${d.option.expiry} • Strike ${d.option.strike} • ${d.option.type}`;$("liquidity").textContent=`Liquidity ${d.option.liquidity} • Volume ${d.option.volume} • OI ${d.option.oi} • BidQty ${d.option.bid_qty||0} • AskQty ${d.option.ask_qty||0} • Spread ${fmt(d.option.spread_pct)}%`;$("optionLtp").textContent="₹"+fmt(d.option.ltp);$("bid").textContent="₹"+fmt(d.option.bid);$("ask").textContent="₹"+fmt(d.option.ask);$("entry").textContent="₹"+fmt(d.option.entry);$("sl").textContent="₹"+fmt(d.option.sl);$("t1").textContent="₹"+fmt(d.option.target1);$("t2").textContent="₹"+fmt(d.option.target2)}else{["contract","expiry","liquidity","optionLtp","bid","ask","entry","sl","t1","t2"].forEach(id=>$(id).textContent="—")}
$("dot").className="dot on";$("status").textContent="Connected • V2.4";$("error").textContent=d.warning||"";monitorTrade(d);updateTradeUI(readTrade())}
...
def confirm_trigger(direction,spot,atr,strong):
    """Require two consecutive refreshes beyond a frozen breakout level."""
    global _trigger_state

    if not strong or direction not in ("BUY","SELL"):
        _trigger_state={"direction":None,"level":None,"count":0,"confirmed":False}
        return None,0,False

    # New directional setup -> freeze one breakout level.
    if _trigger_state["direction"]!=direction or _trigger_state["level"] is None:
        ...
        _trigger_state={"direction":direction,"level":round(level,2),"count":0,"confirmed":False}
    ...
    return level,min(_trigger_state["count"],2),bool(_trigger_state["confirmed"])
...
def build_signal():
    ...
    # V2.4: freeze a trigger when a strong setup appears and require
    # two consecutive server refreshes beyond it (roughly 24-30 sec).
    trigger_level,confirm_count,trigger_confirmed=confirm_trigger(
        direction,tv["spot"],tv["atr5"],strong
    )
    ...
    if strong:
        ...
        if execution_ready:
            signal=direction
            bias=("BULLISH" if bull else "BEARISH")+" • TRIGGER CONFIRMED 2/2"
        else:
            signal=("BUY WATCH" if bull else "SELL WATCH")
            waiting=[]
            if not trigger_confirmed:
                waiting.append(f"NIFTY trigger confirmation {confirm_count}/2")
            if not opt:
    ...
    elif raw_signal!="NO TRADE":
        confirm_count=0;trigger_confirmed=False

    return {
        "spot":round(tv["spot"],2),"signal":signal,"bias":bias,"confidence":conf,
        "reason":f"{conf}/100 • 5m {tv_rating(tv['rec5'])} • 15m {tv_rating(tv['rec15'])} • ADX {tv['adx5']:.1f} • Trigger {confirm_count}/2",
