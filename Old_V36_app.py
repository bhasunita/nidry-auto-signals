...
def health():
    return jsonify({"ok": True, "version": "3.7"})
...
def instrument_config(key):
    ...
    return key, INSTRUMENTS.get(key, INSTRUMENTS["NIFTY"])
...
<meta name="theme-color" content="#07111f"><title>Professional Signals V3.7</title>
<style>
...
<h1>📈 Professional Signals V3.7</h1><div class="sub">NIFTY options + MCX futures • 5m + 15m confirmation • quality filters • trade alerts</div>
...
<div class="card"><div class="small">V3.7 MARKET / INSTRUMENT</div><div class="selectrow" style="margin-top:8px"><select id="marketSelect"><option value="NSE">NIFTY OPTIONS</option><option value="MCX">MCX COMMODITIES</option></select><select id="instrumentSelect"></select></div><div class="modehint" id="instrumentHint">NIFTY option mode uses NSE option-chain quotes.</div></div>
...
<div class="small">V3.7 PROFESSIONAL ENTRY QUALITY ENGINE</div>
...
<div class="card" id="tradeHealthCard"><div class="small">V3.7 PROFESSIONAL TRADE HEALTH</div>
...
<div class="small">V3.7 END-OF-DAY POSITION DECISION</div>
...
$("marketSelect").onchange=()=>{selectedInstrument=$("marketSelect").value==="NSE"?"NIFTY":"CRUDEOILM";setInstrumentOptions();localStorage.setItem("v37Instrument",selectedInstrument);clearTrade();refresh()};
$("instrumentSelect").onchange=()=>{selectedInstrument=$("instrumentSelect").value;localStorage.setItem("v37Instrument",selectedInstrument);clearTrade();refresh()};
...
function render(d){
 ...
 $("updated").textContent=`Updated ${d.updated} • ${d.data_source}`; document.querySelector("h1").textContent=`📈 ${d.instrument_label||"Professional"} Signals V3.7`;
...
def api_commodity_quote():
    try:
        return jsonify(commodity_quote(request.args.get("instrument") or "CRUDEOILM"))
    except Exception as e:
        return jsonify({"error":str(e)}),503
...
def api_signal():
    ...
    now=time.time(); c=_cache.get(instrument)
    if c and c.get("signal") is not None and now-c.get("ts",0)<12:return jsonify(c["signal"])
    try:
        r=build_signal(instrument);_cache[instrument]={"signal":r,"ts":now};return jsonify(r)
    except Exception as e:
        c=_cache.get(instrument)
        if c and c.get("signal") is not None:
            x=dict(c["signal"]);x["warning"]="Using cached data: "+str(e);return jsonify(x)
        return jsonify({"error":str(e)}),503
